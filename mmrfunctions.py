...
def update_comp():
    ...
    for num in range(1,15)
        for key in dict_comp.iterkeys():
            if dict_comp[key]['position'] == str(num):
                text = "%s %s: %s %s\n" % (text, key,dict_comp[key]['first_name'],dict_comp[key]['last_name'])
...
def clear_dicts():
    dict_comp = {'number' : {}, 'first_name' : {}, 'last_name' : {},'transponder' : {}, 'position' : {}}
    

def get_number_of_laps():
        pass
