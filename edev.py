...
if not shell.AppActivate(osb_fgw_name):
        ...
        if shell.AppActivate(osb_fgw_name):
                ...
                print "unable to launch obs with focus window check OBS is installed and correct profile is set to default"
                sys.exit(1)
...
def start_recording(): #Opens and virtually presses button to start recording
    ...
    if wsh.AppActivate(osb_fgw_name):
        ...
        dest = "C:\\placeholder.txt"
        sql = "insert into race_data (name, start, file) values ('%s', '%s', '%s')" % (race_name,int(time.time()),dest)
        cursor.execute(sql)
        cnx.commit()
        is_recording = True
